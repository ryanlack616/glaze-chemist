...
def build_people(data, all_books):
    ...
    for b in all_books:
        for aid in b.get("author_ids", []):
            books_by_author.setdefault(aid, []).append(b)
...
document.getElementById("filterInput").addEventListener("input", function() {{
  ...
  {items}
        ...
        if collabs:
            ...
            collab_html = f'''<div class="section">
...
(function() {{
  ...
  canvas.addEventListener("mousedown", (ev) => {{
    ...
    if (n) {{
      dragging = n;
      const rect = canvas.getBoundingClientRect();
      dragOffset.x = n.x - (ev.clientX - rect.left);
      dragOffset.y = n.y - (ev.clientY - rect.top);
  ...
  canvas.addEventListener("mousemove", (ev) => {{
    if (dragging) {{
      const rect = canvas.getBoundingClientRect();
      dragging.x = ev.clientX - rect.left + dragOffset.x;
      dragging.y = ev.clientY - rect.top + dragOffset.y;
...
def build_index(data):
  ...
  <a href="people/{make_person_slug(p)}.html">
    ...
    fb_cards = []
    people_by_id = {pp["id"]: pp for pp in people}
